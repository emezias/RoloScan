 ...
 */
class SetContactFieldsActivity : AppCompatActivity(), OnItemSelectedListener {
    val editFields = intArrayOf(R.id.cc_edit1, R.id.cc_edit2, R.id.cc_edit3, R.id.cc_edit4, R.id.cc_edit5,
            R.id.cc_edit6, R.id.cc_edit7, R.id.cc_edit8, R.id.cc_edit9, R.id.cc_edit10)
    val spinners = intArrayOf(R.id.cc_spinner1, R.id.cc_spinner2, R.id.cc_spinner3, R.id.cc_spinner4, R.id.cc_spinner5,
            R.id.cc_spinner6, R.id.cc_spinner7, R.id.cc_spinner8, R.id.cc_spinner9, R.id.cc_spinner10)
    val btn_fields = intArrayOf(R.id.cc_btn1, R.id.cc_btn2, R.id.cc_btn3, R.id.cc_btn4, R.id.cc_btn5,
    ...
    override fun onResume() {
        ...
        mContactFields = intent.extras?.getStringArray(TAG) as Array<String> //scanned text
        if (mContactFields.isNullOrEmpty()) {
            showSnackbar(cc_spinner1, R.string.no_text)
            finish()
            return
        }
        val spinDexes = ContactSpinnerAdapter.getIndices(mContactFields)
        ...
        for (i in mContactFields.indices) {
            ...
            text = findViewById<View>(editFields[i]) as EditText
            btn = findViewById<View>(btn_fields[i]) as ImageButton
            text.addTextChangedListener(EditButton(btn))
            spinner = findViewById<View>(spinners[i]) as Spinner
    ...
    fun saveContact(v: View?) {
        ...
        for (dex in editFields.indices) {
            value = (findViewById<View>(editFields[dex]) as EditText).text.toString()
            if (!TextUtils.isEmpty(value)) {
                spinnerDex = (findViewById<View>(spinners[dex]) as Spinner).selectedItemPosition
    ...
    fun clearText(btn: View) {
        btn.isEnabled = false
        (findViewById<View>(editFields[(btn.tag as Int)]) as EditText).text.clear()
    }
    ...
    private fun createContact(contactMap: HashMap<Int, String>) {
        ...
        for (key in contactMap.keys) {
            when (key) {
                ContactSpinnerAdapter.IND_NAME ->                     //could not find the right combination of content value keys for the name
                ...
                ContactSpinnerAdapter.IND_ADDR -> {
                    ...
                    contactdata.put(ContactsContract.CommonDataKinds.StructuredPostal.FORMATTED_ADDRESS, contactMap[key])
                    data.add(contactdata)
                }
                ContactSpinnerAdapter.IND_COMPANY, ContactSpinnerAdapter.IND_TITLE -> {
                    //if (company != null) break
                    contactdata = ContentValues()
                    company = contactMap[ContactSpinnerAdapter.IND_COMPANY]
                    ...
                    company = contactMap[ContactSpinnerAdapter.IND_TITLE]
                    if (!TextUtils.isEmpty(company)) {
    ...
    fun showDuplicatesDialog(duplicates: ArrayList<Int>, contactMap: HashMap<Int, String>) {
        val message = StringBuilder("Dropping duplicated values:\n")
        for (id in duplicates) {
            message.append((findViewById<View>(id) as EditText).text.toString()).append("\n")
        }
